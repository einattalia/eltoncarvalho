...
## v15.1 — correção do erro 500 no Admin
- O Admin agora valida a sessão primeiro e não bloqueia a entrada se um módulo secundário falhar.
- A consulta de demandas usa `select=*`, evitando erro 500 por colunas opcionais ausentes.
- Aceita `SUPABASE_PUBLISHABLE_KEY` ou a chave legada `SUPABASE_ANON_KEY`.
- Execute `supabase/migration-v15-admin-compat.sql` no SQL Editor do Supabase para garantir compatibilidade do banco.
